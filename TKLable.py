...
def search():
    if rightsearch.get() == "":
        ...
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="AKASHSUNNY143@", database="STUDENTDATA")
            curr = conn.cursor()
...
def insert_funct():
    if rollno.get() == "" or name.get() == "":
        ...
        conn = mysql.connector.connect(host="localhost", user="root", password="urpassword", database="STUDENTDATA")
        curr = conn.cursor()
